...
fn main() {
    ...
    // Предварительный расчет векторов ребер для перевода в глобальные координаты
    let e1_vec = triangle.p1 - triangle.p0;
    let e2_vec = triangle.p2 - triangle.p0;

    for (x, y) in test_points {
        // Вычисляем PT - глобальную координату точки на плоскости треугольника
        ...
        // Выводим pt.x и pt.y. 
        // Так как Z у нашего треугольника равен 0, двух координат достаточно для таблицы отчета
        println!("[{:.1}], [{:.1}], [{:.4}], [{:.4}], [{:.4}],", pt.x, pt.y, e2.x, e2.y, e2.z);
    }
